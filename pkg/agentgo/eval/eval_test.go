...
func TestEvaluator_EvaluateModel(t *testing.T) {
	...
	if sum.Runs != 2 || sum.Successes != 2 || sum.Failures != 0 {
		t.Fatalf("unexpected summary: %+v", sum)
	}
	if sum.AvgPromptTokens != 5 {
		t.Fatalf("avg prompt tokens = %v, want 5", sum.AvgPromptTokens)
	}
	if sum.AvgCompletion != 7 {
		t.Fatalf("avg completion tokens = %v, want 7", sum.AvgCompletion)
	}
}
...
func TestEvaluator_CompareModels(t *testing.T) {
	...
	if len(res) != 2 {
		t.Fatalf("results models=%d, want 2", len(res))
	}
	if len(cmp.Models) != 2 {
		t.Fatalf("cmp models=%d, want 2", len(cmp.Models))
	}
}
