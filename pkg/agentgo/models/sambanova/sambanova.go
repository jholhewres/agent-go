...
func New(modelID string, config Config) (*SambaNova, error) {
	if config.APIKey == "" {
		return nil, types.NewInvalidConfigError("SambaNova API key is required", nil)
	}
...
func (s *SambaNova) Invoke(ctx context.Context, req *models.InvokeRequest) (*types.ModelResponse, error) {
	...
	if err != nil {
		return nil, types.NewAPIError("failed to call SambaNova API", err)
	}
	if len(resp.Choices) == 0 {
		return nil, types.NewAPIError("no response from SambaNova", nil)
	}
	ch := resp.Choices[0]
	...
	go func() {
		...
		for {
			...
			for _, choice := range resp.Choices {
				if d := choice.Delta; d.Content != "" {
					ch <- types.ResponseChunk{Content: d.Content}
				}
			}
		}
	}()
...
func (s *SambaNova) buildChatRequest(req *models.InvokeRequest) openai.ChatCompletionRequest {
	...
	for i, m := range req.Messages {
		...
		if len(m.ToolCalls) > 0 {
			...
			for j, tc := range m.ToolCalls {
				calls[j] = openai.ToolCall{ID: tc.ID, Type: openai.ToolType(tc.Type), Function: openai.FunctionCall{Name: tc.Function.Name, Arguments: tc.Function.Arguments}}
			}
			msg.ToolCalls = calls
		}
	...
	if len(req.Tools) > 0 {
		...
		for i, td := range req.Tools {
			tools[i] = openai.Tool{Type: openai.ToolType(td.Type), Function: &openai.FunctionDefinition{Name: td.Function.Name, Description: td.Function.Description, Parameters: td.Function.Parameters}}
		}
		chatReq.Tools = tools
	}
